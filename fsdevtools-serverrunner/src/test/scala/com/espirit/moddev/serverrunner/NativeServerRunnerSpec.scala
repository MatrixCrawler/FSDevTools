...
class NativeServerRunnerSpec extends WordSpec with Matchers with Eventually {
  def fixture = new {
    lazy val propsWithVersionBuilder        = ServerProperties.builder().serverRoot(new File(System.getProperty("fsServerRoot")).toPath).firstSpiritJar(FirstSpiritJar.SERVER, new File("foobar"))
  ...
  "NativeServerRunner.prepareStartup" should {
    ...
    "only contain '-Xloggc' when it is explicitly configured" in {
      ...
      val argsGcLog =
        NativeServerRunner
          .prepareStartup(
            ServerProperties.builder().serverInstall(false).serverGcLog(true).firstSpiritJar(FirstSpiritJar.SERVER, new File("foobar")).build())
          .asScala
    ...
    "add the server ops" in {
      val commands = NativeServerRunner
        .prepareStartup(
          ServerProperties.builder().serverOp("-Dabc=123").serverOp("-Dcde=234").firstSpiritJar(FirstSpiritJar.SERVER, new File("foobar")).build())
